...
import styles from '../css/InstituicoesProximas.module.css';

const InstituicoesProximas = () => {
  ...
  return (
    <div className={styles.container} id='instituicoesProximas'>
      <div className={styles.header}>
        <h1 className={styles.titulo}>Encontre instituições próximas de você</h1>
        <p className={styles.subtitulo}>Use sua localização para descobrir instituições recomendadas na sua região.</p>
      </div>
          ...
          <>
            <div className={styles.cardsContainer}>
              {instituicoesPaginaAtual.map((instituicao) => (
                <div key={instituicao.id} className={styles.card}>
                  ...
                  <div className={styles.cardConteudo}>
                    <h3 className={styles.cardTitulo}>{instituicao.nome}</h3>
                    <p className={styles.cardLocalizacao}>{instituicao.cidade} - {instituicao.estado}</p>
                    <p className={styles.cardDescricao}>{instituicao.descricao}</p>
                    {instituicao.site && (
                      <a 
                        href={instituicao.site} 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className={styles.botaoSite}
                      >
                        Visitar Site
                      </a>
                    )}
              ...
              {instituicoesPaginaAtual.length < 2 && (
                <div className={`${styles.card} ${styles.cardSugestao}`}>
                  <div className={styles.cardImagemContainer}>
                    <div className={styles.iconeSugestao}>+</div>
                  </div>
                  <div className={styles.cardConteudo}>
                    <h3 className={styles.cardTitulo}>Sugira uma instituição</h3>
                    <p className={styles.cardDescricao}>Conhece alguma instituição que não está listada? Ajude-nos a melhorar nosso cadastro!</p>
                    <button 
            ...
            {totalPaginas > 1 && (
              <div className={styles.paginacao}>
                ...
                {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((num) => (
                  <button
                    key={num}
                    className={`${styles.botaoPaginacao} ${paginaAtual === num ? styles.ativo : ''}`}
...
export default InstituicoesProximas